...
func _run() -> void:
    ...
    _check(avatar != null, "LumAvatar instantiates as Node3D")
    if avatar == null:
        _finish()
        return
    ...
    var eye_mode: String = String(summary.get("eye_tracking_mode", ""))
    ...
    var target: Vector3 = avatar.global_position + Vector3(0.5, 1.9, -3.5)
    avatar.call("set_look_target_world", target)
    await process_frame
    var marker: Node3D = avatar.get_node_or_null("LookTarget") as Node3D
